...
from utils.data_utils import Vocab, Data2tensor, Csvfile, seqPAD, Embeddings
seed_num = 12345
...
class Classifier(object):
    def __init__(self, args=None):
        ...
        self.model = fNN(word_HPs=word_HPs, num_labels=len(self.args.vocab.l2i)).to(self.device)
    ...
    def evaluate_batch(self, eva_data):
        with torch.no_grad():
            ...
            for i,(words, asp_locs, label_ids) in enumerate(self.args.vocab.minibatches(eva_data, batch_size=batch_size)):
                word_ids, sequence_lengths = seqPAD.pad_sequences(words, pad_tok=0, wthres=wl)
        
                data_tensors = Data2tensor.sort_tensors(label_ids, asp_locs, word_ids,sequence_lengths, self.device)
                label_tensor, aspect_tensor, word_tensor, sequence_lengths, word_seq_recover = data_tensors
                arange_tensor = Data2tensor.idx2tensor(list(range(word_tensor.size(0))), self.device)

                y_true = torch.cat([y_true,label_tensor])
                label_score = self.model(word_tensor, sequence_lengths, aspect_tensor, arange_tensor)
...
#        print("Gradient flag: ", label_score.requires_grad)
    ...
    def train_batch(self,train_data):
        ...
        self.model.train()
        train_loss = []
        for i,(words, asp_locs, label_ids) in enumerate(self.args.vocab.minibatches(train_data, batch_size=batch_size)):
            word_ids, sequence_lengths = seqPAD.pad_sequences(words, pad_tok=0, wthres=wl)
...
#        train_data = [(tuple(datum[0]),datum[1]) for datum in train_data]
#        dev_data = [(tuple(datum[0]),datum[1]) for datum in dev_data]
#        test_data = [(tuple(datum[0]),datum[1]) for datum in test_data]
    ...
    def predict(self, sent, asp="null", k=1):
        ...
        data_tensors = Data2tensor.sort_tensors(fake_label, [asp_loc], word_ids, sequence_lengths, self.device)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--train_file', help='Trained file', default="/media/data/aspectSA/train_v3.csv", type=str)

    argparser.add_argument('--dev_file', help='Developed file', default="/media/data/aspectSA/val_v3.csv", type=str)

    argparser.add_argument('--test_file', help='Tested file', default="/media/data/aspectSA/test_v3.csv", type=str)

    argparser.add_argument('--emb_file', help='Embedding file',
                           default="/Users/duytinvo/Projects/aspectSA/data/w2v/sf_review.pro.vec", type=str)
    ...
    argparser.add_argument("--word_dim", type=int, default=128, help="word_embedding vector size")
        
    argparser.add_argument("--word_hidden_dim", type=int, default=256, help="LSTM word_hidden layers")
    ...
    argparser.add_argument('--model_name', type=str, default="./results/sf_lstm_v3.m", help='Model name')
    
    argparser.add_argument('--model_args', type=str, default="./results/sf_lstm_v3.args", help='Model arguments')
